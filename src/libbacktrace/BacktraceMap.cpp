...
BacktraceMap::BacktraceMap(pid_t pid) : pid_(pid) {
  ...
  ALOGV("BacktraceMap::BacktraceMap %d", pid_);
}
...
bool BacktraceMap::ParseLine(const char* line, backtrace_map_t* map) {
  ALOGV("BacktraceMap::ParseLine %s", line);
...
// 0         1         2         3         4         5
  ...
  ALOGD("Parsed map: start=%p, end=%p, flags=%x, name=%s",
...
bool BacktraceMap::Build() {
  ...
  ALOGV("BacktraceMap::Build");
#endif
  if (fp == nullptr) {
    ALOGV("BacktraceMap::Build failed for fopen %s faild", path);
    return false;
  }
...
BacktraceMap* BacktraceMap::Create(pid_t pid, bool /*uncached*/) {
  ALOGV("BacktraceMap::Create apple?");
